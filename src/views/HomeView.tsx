...
export const HomeView: React.FC = () => {
  ...
  // 6 songs per section (2 columns x 3 rows)
  const totalRecentPages = Math.min(5, Math.max(1, Math.ceil(recentlyPlayed.length / 6)));
  ...
  const scrollToRecentPage = (pageIdx: number) => {
    const container = recentCarouselRef.current;
    if (!container) return;
    const targetChild = container.children[pageIdx] as HTMLElement;
    if (targetChild) {
      container.scrollTo({
  ...
  return (
    <div className="flex flex-col gap-10 pb-20 select-none">
      {/* SECTION 1 (TOP): Recently Played Horizontal 3-Row Carousel */}
        ...
        <section className="flex flex-col gap-4">
          <div className="flex items-center justify-between">
            ...
            <div className="flex items-center gap-1">
              <button
                type="button"
                onClick={() => scrollToRecentPage(Math.max(0, recentPage - 1))}
                ...
                aria-label="Previous recently played songs"
              >
                <ChevronLeft className="w-4 h-4" />
              </button>
              <button
                type="button"
                onClick={() => scrollToRecentPage(Math.min(totalRecentPages - 1, recentPage + 1))}
                disabled={recentPage >= totalRecentPages - 1}
                className="p-1.5 rounded-full bg-white/5 hover:bg-white/10 text-white/50 hover:text-white disabled:opacity-20 transition-all cursor-pointer"
                aria-label="Next recently played songs"
              >
          ...
          {/* Paginated 6-Songs-per-Section (2-Cols x 3-Rows) Hardware-Accelerated Carousel */}
          <div
            ref={recentCarouselRef}
            onScroll={handleRecentScroll}
            className="flex overflow-x-auto snap-x snap-mandatory scrollbar-none overscroll-x-contain pb-2 touch-pan-x gpu-layer"
            style={{
          ...
          >
            {Array.from({ length: totalRecentPages }).map((_, pageIdx) => {
              ...
              return (
                <div
                  key={pageIdx}
                  className="min-w-full w-full flex-shrink-0 snap-start snap-always grid grid-cols-2 grid-rows-3 gap-2 sm:gap-2.5 px-0.5"
                >
